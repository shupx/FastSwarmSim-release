...
namespace marsim {
    ...
    void MarsimRender::read_pointcloud_fromfile(const std::string& map_filename) {
        ...
        const auto vertex_path = resource_paths_.shader_directory / "360camera.vs";
        const auto fragment_path = resource_paths_.shader_directory / "camera.fs";
        std::cout << "Load shader frome: " << vertex_path << std::endl;;
        ourShader = Shader(vertex_path.string().c_str(), fragment_path.string().c_str());
