...
namespace marsim {
    ...
    struct ResourcePaths {
        std::filesystem::path pcd_directory;
    ...
    public:
        ...
        Config(const std::string& cfg_path, const ResourcePaths& resource_paths) {
            ...
            if (!pcd_name.empty()) {
                const std::filesystem::path configured_path(pcd_name);
                pcd_name = configured_path.is_absolute() ?
                    configured_path.string() :
                    (resource_paths.pcd_directory / configured_path).string();
            }
